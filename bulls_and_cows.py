        ...
        if message.text != 'Cancel':
            if filter_users_number(message.text):
                ...
                if bulls == 4:
                    ...
                    await message.answer(f'You won! It took {turns} turns.')
                    await state.finish()
                else:
...
@dp.callback_query_handler(Acceptc.filter(), state=Project_state.Call_user)
async def accept(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await state.set_state(Project_state.UserNum)
    await call.answer()

@dp.callback_query_handler(text="reject", state=Project_state.Call_user )
async def accept(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
